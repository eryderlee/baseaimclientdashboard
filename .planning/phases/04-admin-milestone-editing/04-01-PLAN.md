...
must_haves:
  truths:
    ...
    - "startDate auto-sets when status changes from NOT_STARTED to IN_PROGRESS"
  artifacts:
    - path: "lib/dal.ts"
...
3. For each milestone in validated data, determine if startDate needs updating:
...
4. Use `prisma.$transaction()` with array of `prisma.milestone.update()` calls. For each milestone update: set `status`, `dueDate` (parse string to Date or null), `notes`, and conditionally `startDate`/`completedAt` as determined above. Also recalculate and store `progress` using `calculateMilestoneProgress` from the progress utility.
5. After successful transaction, call `revalidatePath('/dashboard/progress')` and `revalidatePath('/dashboard/admin/clients/' + clientId)` to bust cache for both admin and client views.
6. Return `{ success: true }` on success, `{ error: 'Failed to update milestones' }` on catch.

To determine if status is "changing", fetch current milestones first with a quick `prisma.milestone.findMany({ where: { clientId }, select: { id: true, status: true, startDate: true } })` before the transaction.
...
Run `npx tsc --noEmit` to verify TypeScript compiles. Verify the file contains `'use server'` directive, Zod validation, `verifySession()` call, `$transaction`, and `revalidatePath` calls.
  ...
  <done>
Server Action validates admin role, validates input with Zod, auto-sets startDate/completedAt on status transitions, batch-updates milestones in a Prisma transaction, recalculates progress percentages, and revalidates both admin and client dashboard paths.
  </done>
</task>
...
<success_criteria>
All three files compile cleanly. DAL functions enforce admin role. Server Action handles the full lifecycle: auth check, validation, status-aware date transitions, batch transaction, progress recalculation, and cache revalidation.
</success_criteria>
