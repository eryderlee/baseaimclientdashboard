...
must_haves:
  ...
  artifacts:
    - path: "app/admin/clients/[clientId]/edit/page.tsx"
      ...
      contains: "ClientForm"
    - path: "components/admin/status-toggle-button.tsx"
      provides: "Client component for toggling client active/inactive status"
  ...
  key_links:
    ...
    - from: "app/admin/page.tsx"
      to: "components/admin/status-toggle-button.tsx"
      via: "import { StatusToggleButton } from '@/components/admin/status-toggle-button'"
      pattern: "import.*StatusToggleButton.*from.*components/admin/status-toggle-button"
    - from: "components/admin/status-toggle-button.tsx"
...
<task type="auto">
  ...
  <action>
    **Route structure clarification:**
    - `/admin/clients/[clientId]` = existing milestone editing page (page.tsx already exists, do NOT modify)
    - `/admin/clients/[clientId]/edit` = NEW client details editing page (this task creates it)
    These are two distinct pages serving different purposes. The `/edit` nested route is intentional.
    ...
    2. Create `components/admin/status-toggle-button.tsx` as a separate `'use client'` component file:
       - Export `StatusToggleButton` accepting `{ clientId: string, isActive: boolean }` props
    ...
    3. Update `app/admin/page.tsx` client table:
       - Add this import at the top: `import { StatusToggleButton } from '@/components/admin/status-toggle-button'`
       ...
       - Add three action buttons per client row wrapped in `<div className="flex items-center gap-1">`:
         ...
         c. `<StatusToggleButton clientId={client.id} isActive={client.isActive} />` -- the imported client component
  </action>
  <verify>
    - `npx tsc --noEmit` passes
    - `npm run build` completes without errors
    - Verify StatusToggleButton import in admin page: `grep "StatusToggleButton" app/admin/page.tsx` shows the import from `@/components/admin/status-toggle-button`
  </verify>
  <done>
    - /admin/clients/[clientId]/edit page renders ClientForm pre-filled with client data
    - StatusToggleButton exists as separate client component at components/admin/status-toggle-button.tsx
    - Admin dashboard imports StatusToggleButton and renders Edit, Milestones, and Deactivate/Activate buttons per client
