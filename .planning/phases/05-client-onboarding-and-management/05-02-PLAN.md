...
<task type="auto">
  ...
  <action>
    ...
    **Mode switching complexity note:** The form uses conditional types and schemas based on `mode` prop. Key pattern:
    - Determine schema at top of component: `const schema = mode === 'create' ? createClientSchema : updateClientSchema`
    - Determine resolver: `zodResolver(schema)`
    - Conditionally render email/password fields only when `mode === 'create'`
    - Submit handler branches on mode to call `createClient` vs `updateClient`
    Keep the branching logic clean -- use early returns or clear if/else blocks, not nested ternaries.
