...
12. [Netlify Deployment Compatibility](#netlify-deployment-compatibility)
...
### Target Audience (ICP)
- **Primary**: Accounting firms, CPA practices, tax professionals
- **Core offering**: Complete client acquisition systems — funnels, landing pages, and paid ads
- **Key messaging**: Predictable growth, escaping referral dependency, 5+ qualified consultation calls per month
- **Tone of copy**: Direct, outcome-focused, empathetic to the referral struggle
...
| **Brand Red/Orange/Yellow** | Special highlights, badges | Performance metrics, guarantees |
| **Grays** | Text hierarchy, borders | Body text, dividers, subtle backgrounds |
| **Success Green** | Success states, positive metrics | Form success, qualified lead counts |
...
```
feat: Add lead qualification form component
fix: Correct gradient animation on mobile
style: Update CTA button hover states
refactor: Simplify booking form validation logic
perf: Optimize wave layer animations
docs: Update component documentation
test: Add tests for pricing calculator
...
**ICP context**: This website targets accounting firms, CPA practices, and tax professionals. All copy should speak to their pain points (referral dependency, unpredictable revenue, time spent on marketing) and position Baseaim as building complete client acquisition systems (funnels, landing pages, paid ads) with a performance guarantee of 5+ qualified consultation calls per month.
...
9. **Copy**: Keep all messaging aligned to the accounting firm ICP — avoid generic AI/automation language
...
If deploying a site built from this style guide to **Netlify**, add the following configuration to avoid common issues.

### 1. Static Export (Required)

Next.js must output static files for Netlify. Add to `next.config.js`:
...
Create `public/_headers` for basic security headers:
...
Set any environment variables (API keys, service URLs) in the **Netlify Dashboard** under:
**Site Settings > Environment Variables** — not in `.env` files committed to the repo.

### 5. Serverless Functions (Optional)

If the site needs backend logic (form handling, API proxying), create functions in `netlify/functions/`:

```
netlify/
  functions/
    contact.js    # Example: form submission handler
```
...
### Quick Netlify Checklist

- [ ] `output: 'export'` set in `next.config.js`
- [ ] `netlify.toml` present with correct publish dir (`out`) and build command
- [ ] SPA redirect rule added for client-side routing
- [ ] `public/_headers` file with security headers
- [ ] Environment variables configured in Netlify Dashboard (not `.env`)
- [ ] Test deploy with `netlify deploy --prod` or via Git push
